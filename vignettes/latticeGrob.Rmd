...
该图使用的是R包latticeGrob中的levelplot2绘制。
...
latticeGrob中设计了`panel.annotation`函数，便捷地为子图添加插件。`panel.annotation`主要是将grob对象绘制到viewpoint的指定位置。调用方法如下：
``` r
panel.annotation(
  grob,
...
latticeGrob处于频繁维护期，尚未发布稳定版。想要尝鲜的用户，需要关注微信号，回复latticeGrob。
